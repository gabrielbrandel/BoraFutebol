...
const messages = ref<{ id: string; userId: string; userName: string; content: string; createdAt: string }[]>([])
...
async function loadMatch() {
  ...
  if (matchRes.data.success) match.value = matchRes.data.data
  if (attRes.data.success) attendances.value = attRes.data.data
}

async function confirmPresence() {
  actionLoading.value = true
  try {
    const { data } = await matchesApi.confirm(matchId.value)
    if (data.success) { toast.add({ severity: 'success', summary: 'Presença confirmada!' }); await loadMatch() }
  } catch (e: unknown) {
...
async function generateTeams() {
  const { data } = await matchesApi.generateTeams(matchId.value)
  if (data.success) teams.value = data.data
}

async function loadMessages() {
  const { data } = await matchesApi.getMessages(matchId.value)
  if (data.success) messages.value = data.data.data || data.data
}
